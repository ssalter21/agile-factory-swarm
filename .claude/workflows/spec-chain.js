...
if (mode === 'fold') {
  ...
  const folded = await agent(
    [
      ...
      'brief.md or the front matter. A run may not approve its own output (section 11).',
...
const written = await agent(
  [
    ...
    'Front matter is status: draft and revision: 1. You never write approved: a run may not approve',
    'its own output (\u00a711).',
